                ...
                @foreach ($actions as $action)
                  @if ($action === 'show')
                    <a class="mx-1 text-lg" role="button"
                      href="{{ route('admin.promocodes.show', $promocode->id) }}">
                      <x-icon.eye class="w-5 h-5" />
                    </a>
                  @elseif ($action === "edit")
                    <a class="mx-1 text-lg" role="button"
                      href="{{ route('admin.promocodes.edit', $promocode->id) }}">
                      <x-icon.pencil-alt class="w-5 h-5" />
                    </a>
                  @elseif ($action === "delete")
                    <a class="mx-1 text-lg" role="button"
                      wire:click="showModal('{{ Crypt::encrypt($promocode->id) }}')">
          ...
          @else
            <x-ui.table-data>
              @if (array_key_exists('relation', $column) && isset($column['relation']))
                @if (array_key_exists('format', $column) && isset($column['format']))
                  @if (count($column['format']) > 1)
                    {{ $column['format'][0](data_get($promocode, $column['relation']), implode(',', array_slice($column['format'], 1))) }}
                  @else
              ...
              @else
                @if (array_key_exists('format', $column) && isset($column['format']))
                  @if (count($column['format']) > 1)
                    {{ $column['format'][0](data_get($promocode, $column['field']), implode(',', array_slice($column['format'], 1))) }}
                  @else
