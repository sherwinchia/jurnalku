    ...
    <tbody>
        ...
        <tr>
            ...
            <x-ui.table-data>
                ...
                {{ $column["format"][0](data_get($user,$column["relation"]), implode(",", array_slice($column["format"],
                1))) }}
                @else
                ...
                @if (array_key_exists("format", $column) && isset($column["format"]))
                @if (count($column["format"]) > 1)
                {{ $column["format"][0](data_get($user,$column["field"]), implode(",", array_slice($column["format"],
                1))) }}
                @else
                {{ $column["format"][0](data_get($user,$column["field"])) }}
                @endif
                @else
